...
keywords: ["livepeer", "resources", "documentation guide", "style guide", "styling", "mintlify", "gotchas", "best practices"]
...
The `.whitelist` file is protected by pre-commit checks.

If a human intentionally needs to edit `.whitelist`, use:
...
The test suite runs automatically in pre-commit hooks (fast mode) and in CI/CD (full suite).

...
- `v2/pages/<top-level-section>/index.mdx` for each direct child folder under `v2/pages/`
- `v2/pages/index.mdx` as the root aggregate index
...
- Links are rendered in markdown link-list format (`- [Title](path/file.mdx)`).
- Links include a warning marker when a page is missing from `docs.json` navigation:
  `- [⚠️ Title](path/file.mdx)`.
...
- Runs in pre-commit when staged files include `v2/pages` changes:

...
description: 'Learn how to authenticate requests to the Livepeer Studio API'
keywords: ["livepeer", "studio", "authentication", "api key", "cors"]
...
1. **Use backend API keys by default** - Only use CORS-enabled keys when absolutely necessary
2. **Never commit API keys** - Use environment variables or secure secret management
3. **Rotate keys regularly** - Especially if you suspect a key has been compromised
4. **Use separate keys** - Different keys for development and production environments

...
curl -G -s https://loki.livepeer.report/loki/api/query \
     --data-urlencode "query={region=~\".+\"}" | jq

# logs from all regions between two timestamps (UNIX epoch nanoseconds)
curl -G -s https://loki.livepeer.report/loki/api/query_range \
...
curl -G -s https://loki.livepeer.report/loki/api/label/region/values | jq '.data'
```
